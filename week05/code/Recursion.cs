...
public static class Recursion
{
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        ...
        {
            // Choose the current letter (letters[i]) and append it to the current word
            string newWord = word + letters[i];
            
            // Create a new string excluding the chosen letter and recurse
            string remainingLetters = letters.Substring(0, i) + letters.Substring(i + 1);

            // Recurse with the remaining letters and reduced size
    ...
    {
         // Base case: if the pattern contains no '*', add it to the results list
        if (!pattern.Contains('*'))
        {
            results.Add(pattern);
            return;
        ...
        // Recursive case: find the first '*' and replace it with '0' and '1'
        int index = pattern.IndexOf('*');  // Find the first '*' in the pattern
